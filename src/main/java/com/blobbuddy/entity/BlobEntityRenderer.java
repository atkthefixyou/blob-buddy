package com.blobbuddy.entity;

import com.blobbuddy.BlobBuddyMod;
import net.minecraft.client.model.geom.ModelLayerLocation;
import net.minecraft.client.renderer.entity.EntityRendererProvider;
import net.minecraft.client.renderer.entity.MobRenderer;
import net.minecraft.resources.ResourceLocation;

public class BlobEntityRenderer extends MobRenderer<BlobEntity, BlobEntityModel> {

    public static final ModelLayerLocation LAYER = new ModelLayerLocation(
        new ResourceLocation(BlobBuddyMod.MOD_ID, "blob"), "main");

    public BlobEntityRenderer(EntityRendererProvider.Context ctx) {
        super(ctx, new BlobEntityModel(ctx.bakeLayer(LAYER)), 0.4f);
    }

    @Override
    public ResourceLocation getTextureLocation(BlobEntity entity) {
        return new ResourceLocation(BlobBuddyMod.MOD_ID, "textures/entity/blob_neutral.png");
    }
}
