...
public class BlobEntity extends PathfinderMob implements GeoEntity {
    ...
    // Tên animation khớp với file blob.animation.json
    ...
    private static final RawAnimation ANIM_ATTACK  = RawAnimation.begin().thenPlayAndHold("animation.blob.attack");
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        // Controller riêng cho attack (không loop, chỉ play 1 lần)
    ...
    private void sendMessageNearby(String text) {
        level().players().stream()
            ...
            .forEach(p -> p.sendSystemMessage(
                Component.literal("§d[Blob] §f" + text)
            ));
    }
